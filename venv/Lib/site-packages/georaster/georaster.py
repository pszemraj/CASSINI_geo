        ...
        if nBands == 1:
            ...
            if type(bands) == int:
                if from_ds:
                    ...
                    z_interp = np.array(
                        [b.ReadAsArray(int(xp), int(yp), 1, 1)[0, 0] for (xp, yp) in zip(xi, yi)])
                else:
                ...
                if from_ds:
                    ...
                    for k in xrange(len(bands)):
                        z_interp[:, k] = ndimage.map_coordinates(self.r[:, :, bands[k]], [yi, xi],
                                                                 order=order)
            else:
                    ...
                    for b in self.bands:
                        ...
                        if self.r is None:
                            (tmp, self.extent) = self.read_single_band_subset(load_data,
                                                                              latlon=latlon, extent=True,
                                                                              band=b, update_info=False,
                                                                              downsampl=downsampl)
                        ...
                        # Store subsequent bands in kth dimension of store.
                            ...
                            self.r[:, :, k] = self.read_single_band_subset(load_data, update_info=True,
                                                                           latlon=latlon, band=b,
                                                                           downsampl=downsampl)
